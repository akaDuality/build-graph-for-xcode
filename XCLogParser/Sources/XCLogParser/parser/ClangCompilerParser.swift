...
public class ClangCompilerParser {
    ...
    public func parseTimeTraceFile(_ logSection: IDEActivityLogSection) -> String? {
        ...
        guard hasTimeTraceCompilerFlag(commandDesc: logSection.commandDetailDesc.string) else {
            return nil
        }

        let text = logSection.text.string
    ...
    public func parseLinkerStatistics(_ logSection: IDEActivityLogSection) -> LinkerStatistics? {
        guard hasPrintStatisticsLinkerFlag(commandDesc: String(logSection.commandDetailDesc)) else {
            return nil
        }

        let text = String(logSection.text)
        let range = NSRange(location: 0, length: text.count)
